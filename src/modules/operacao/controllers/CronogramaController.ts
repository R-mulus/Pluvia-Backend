...
class CronogramaController {
  ...
  agendar = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const novoAgendamento = await this.service.agendar({
        ...dadosValidados,
        criado_por,
      });
      res
        .status(201)
        .json({ mensagem: "Cronograma agendado!", dados: novoAgendamento });
    } catch (error) {
      next(error);
    }
  };
  ...
  deletar = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const authReq = req as AuthRequest;
      const operador_id = authReq.user?.id;
      if (!operador_id) throw new Error("Usuário não autenticado");

      await this.service.deletar(req.params.id as string, operador_id);
